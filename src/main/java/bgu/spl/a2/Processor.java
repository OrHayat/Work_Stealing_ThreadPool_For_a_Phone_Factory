...
public class Processor implements Runnable {
    ...
    /*package*/ Processor(int id, WorkStealingThreadPool pool) {
     ...
     void add(Task<?> task)
    {
    	pool.submit(task,this.id);
    }
    @Override
    public void run() {
    	while (!(this.pool.isShutDown||Thread.currentThread().isInterrupted())){
    	Task<?> taskToDo=this.pool.fetch(this.id);
    	if((taskToDo!=null)&&(!this.pool.isShutDown)){
